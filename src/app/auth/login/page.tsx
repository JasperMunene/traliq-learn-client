...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // If backend indicates Google-only, block password flow
        if (methodChecked && !allowPassword && (loginMethod === 'google')) {
            setErrors({ general: 'This account uses Google Sign-In. Please continue with Google.' });
            setIsLoading(false);
            return;
        }
    ...
    const handleGoogleLogin = () => {
        ...
        try {
            // If backend indicates password-only, block Google flow
            if (methodChecked && !allowGoogle && (loginMethod === 'password')) {
                setErrors({ general: 'This email is registered with a password. Please sign in with your password or use “Forgot password”.' });
    ...
    // Debounced login method lookup when email changes
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                const resp = await fetch(API_ENDPOINTS.auth.loginMethod, {
                    method: 'POST',
                ...
                const method = (data?.method as 'unknown' | 'password' | 'google' | 'both') || 'unknown';
                setLoginMethod(method);
                setAllowPassword(Boolean(data?.show_password));
